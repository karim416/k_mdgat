...
def load_kitti_gt_txt(txt_root, seq):
    ...
    :return: [{anc_idx: *, pos_idx: *, seq: *}]                
     '''
    dataset = []
    with open(os.path.join(currentdir+'/KITTI/preprocess-random-full/', '%02d'%seq, 'groundtruths.txt'), 'r') as f:
...
class SparseDataset(Dataset):
    ...
    def __init__(self, opt, mode):
        ...
        for seq in self.seq_list:
            sequence = '%02d'%seq
            calibpath = os.path.join(currentdir+'/KITTI/', 'calib/sequences', sequence, 'calib.txt')
            posepath = os.path.join(currentdir+'/KITTI/', 'poses', '%02d.txt'%seq)
    ...
    def __getitem__(self, idx):
        ...
        if self.memory_is_enough:
            ...
            sequence = '%02d'%seq
            pc_np_file1 = os.path.join(currentdir+'/KITTI/keypoints/tsf_256_FPFH_16384-512-k1k16-2d-nonoise', sequence, '%06d.bin' % (index_in_seq))
            pc_np1 = np.fromfile(pc_np_file1, dtype=np.float32)

            pc_np_file2 = os.path.join(currentdir+'/KITTI/keypoints/tsf_256_FPFH_16384-512-k1k16-2d-nonoise', sequence, '%06d.bin' % (index_in_seq2))
...
if __name__ == '__main__':
    seq=2
  #  with open('./KITTI/preprocess-random-full/02/groundtruths.txt', 'r') as f:
    with open(os.path.join(currentdir+'/KITTI/preprocess-random-full/', '%02d'%seq, 'groundtruths.txt'), 'r') as f:
        lines_list = f.readlines()
